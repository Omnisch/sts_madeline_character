...
- [ ] 登山背包 `Climbing Backpack`：初始遗物。在你的回合结束时，保留所有的跳跃和冲刺。
- [ ] 不知所谓的机器 `Pointless Machines`：你每打出1张冲刺，获得（2？）点格挡。在打出24张冲刺后升级。
- [ ] 沉重与脆弱 `Heavy and Frail` 你每打出3张冲刺，获得12点格挡。

## 姿态

- [ ] 着陆 `Land`：保持不小于1点冲刺机会。失去所有动量。
- [ ] 落脚 `Stay`：保持不小于1点冲刺机会。你打出下一张牌时退出这一姿态。
- [ ] 凌空 `Soar`：在退出这一姿态前，你不能打出跳跃。

## 能力

- [x] 冲刺机会 `Dash Chance` (点)：你需要冲刺机会来打出冲刺。
- [ ] 动量 `Momentum` (层)：移动须额外选择与层数相等的牌。

## 卡牌关键字

- [ ] 移动 `Move`：获得1层动量。选择一张手牌耗能打出，若无法打出则丢弃。
- [ ] 跳跃 `Jump`：进入凌空。
...
- [ ] 原地跳 `Jump in Place` 初始 ①
  > 跳跃着陆。获得【2/3】点临时敏捷，抽1张牌。
- [ ] 立定跳 `Standing Jump` 常见 ⓪
  > 跳跃移动。获得【4/7】点格挡。
- [ ] 兔子跳 `Bunnyhop` 常见 ①
